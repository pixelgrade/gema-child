<?php
/**
 * Gema Child functions and definitions
 *
...
/**
 * Setup Gema Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function gema_child_theme_setup() {
	load_child_theme_textdomain( 'gema-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'gema_child_theme_setup' );
...
function gema_child_enqueue_styles() {
	...
	if ( !is_rtl() ) {
		wp_enqueue_style( 'gema-style', get_template_directory_uri() . '/style.css', array(), $parent->get( 'Version' ) );
	} else {
		wp_enqueue_style( 'gema-style', get_template_directory_uri() . '/rtl.css', array(), $parent->get( 'Version' ) );
	}
	// Here we are adding the child style.css while still retaining
	// all of the parents assets (style.css, JS files, etc)
	wp_enqueue_style( 'gema-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('gema-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);

}

add_action( 'wp_enqueue_scripts', 'gema_child_enqueue_styles' );
...
function gema_child_overwrite_files() {
	...
	// See the parent's function.php -> the gema_scripts_styles() function
	// for details like resources names
	//
	// Remember that the rest of the static resources will still get loaded like 
	// gema-imagesloaded, gema-hoverintent and gema-velocity

		wp_dequeue_script( 'gema-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'gema-child-scripts',
			get_stylesheet_directory_uri() . '/assets/js/main.js',
			array( 'jquery', 'masonry', 'gema-imagesloaded', 'gema-hoverintent', 'gema-velocity' ),
			'1.0.0', true );
	...
	// see the parent's function.php -> the gema_scripts_styles() function for details like resources names
	
		wp_dequeue_style( 'gema-style' );
	...
	// Now you can add your own, modified version of the "style.css" file
		...
		wp_enqueue_style( 'gema-child-style',
			get_stylesheet_directory_uri() . '/style.css',
			array('gema-font-awesome-style') //use the same dependencies as the parent because we want to still use FontAwesome icons
		);
}

// Load the files from the function mentioned above:

	add_action( 'wp_enqueue_scripts', 'gema_child_overwrite_files', 11 );
